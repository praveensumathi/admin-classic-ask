...
const SinglePageBill = ({ productData }) => {
  let totalQuantity = 0;
  productData.sizeWithQuantityPrice.forEach((data) => {
    data.sizes.forEach((sizeData) => {
      totalQuantity += sizeData.billQuantity;
    });
  });
  ...
  productData.sizeWithQuantityPrice.forEach((data) => {
    data.sizes.forEach((sizeData) => {
      totalAmount += sizeData.billQuantity * sizeData.offlineSellingPrice;
    });
  });
  ...
  const totalAmountWithGST = totalAmount + cgstAmount + sgstAmount;

  const generateSerialNumbers = () => {
    return productData.sizeWithQuantityPrice.map((data, index) => index + 1);
  };
  const toWords = new ToWords();
  let wordsInTotalPrice = toWords.convert(totalPrice, { currency: true });
    ...
    <>
      <Container>
        ...
        <Box sx={{ border: "1.5px solid #000" }} mt={2}>
          ...
          {/* Data Row */}
          {productData.sizeWithQuantityPrice.map((data, index) => (
            <Grid container key={index} sx={{ borderBottom: "1px solid #000" }}>
              ...
              ></Grid>
              {data.sizes.map((sizeData, sizeIndex) => (
                // Wrap the JSX elements in curly braces
                <React.Fragment key={sizeIndex}>
                  ...
                  >
                    {(sizeData.offlineSellingPrice * 0.95).toFixed(2)}
                  </Grid>
                  <Grid
                    item
                    md={1}
                  ...
                  >
                    {(sizeData.totalPrice * 0.95).toFixed(2)}
                  </Grid>
                  <Grid
                    item
                    md={2}
